...
def create_variant_bar_chart(variant_data):
    ...
    if include_mixed:
        ...
        text_single.append(f" {mixed_pct}% <b><span style='font-size: 24px;'>🂠+🂠</span></b>" if mixed_pct > 0 else "")
        text_double.append("")  # No double for mixed
    
    # Add var2 data
    single_data.append(var2_single_pct)
    double_data.append(var2_double_pct)
    text_single.append(f" {var2_single_pct}%  <b><span style='font-size: 24px;'>🂠</span></b>" if var2_single_pct > 0 else "")
    text_double.append(f" {var2_double_pct}%  <b><span style='font-size: 24px;'>🂠🂠</span></b>" if var2_double_pct > 0 else "")
    
    # Add var1 data
    single_data.append(var1_single_pct)
    double_data.append(var1_double_pct)
    # FIXED: These were incorrectly showing var2 values
    text_single.append(f" {var1_single_pct}%  <b><span style='font-size: 24px;'>🂠</span></b>" if var1_single_pct > 0 else "")  # Fixed
    text_double.append(f" {var1_double_pct}%  <b><span style='font-size: 24px;'>🂠🂠</span></b>" if var1_double_pct > 0 else "")  # Fixed
