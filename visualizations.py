...
def create_usage_bar_chart(type_cards, card_type):
    ...
    fig.add_trace(go.Bar(
        ...
        text=plot_df['1 Copy'].apply(
            lambda x: f" {format_percentage(x)}   <b><span style='font-size: 16px;'>1</span></b>" if x >= 20 else (f" {format_percentage(x)} " if x > CHART_TEXT_THRESHOLD else '')
        ),
        textposition='inside',
    ...
    fig.add_trace(go.Bar(
        ...
        text=plot_df['2 Copies'].apply(
            lambda x: f" {format_percentage(x)}   <b><span style='font-size: 16px;'>2</span></b>" if x >= 20 else (f" {format_percentage(x)} " if x > CHART_TEXT_THRESHOLD else '')
        ),
        textposition='inside',
        textfont=dict(size=CHART_FONT_SIZE),
        insidetextanchor='start'
