...
def format_card_style(count, pct):
    ...
    if pct <= 0:
        return ""
    return f""" {pct}%  <span style='font-family:Nunito,sans-serif;font-weight:700;font-size:20px'>{count}×</span>"""    
...
def create_variant_bar_chart(variant_data, energy_type=None):
    ...
    text_double.append(format_card_style(2,var1_double_pct) if var1_double_pct > 0 else "")
