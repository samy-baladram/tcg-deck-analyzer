...
def create_usage_bar_chart(type_cards, card_type):
    ...
    fig.add_trace(go.Bar(
        ...
        text=plot_df['1 Copy'].apply(
            lambda x: f" {format_percentage(x)}   <b><span style='font-size: 22px; margin-bottom:-0.2em'>🂠</span></b>" if x >= 20 else (f" {format_percentage(x)} " if x > CHART_TEXT_THRESHOLD else '')
        ),
        textposition='inside',
