...
from config import TOURNAMENT_COUNT, POWER_INDEX_EXPLANATION, POKEMON_EXCEPTIONS
import pandas as pd
import base64
import os
...
def display_deck_header(deck_info, results):
    ...
    if header_image:
        ...
        <div style="display: flex; flex-wrap: wrap; align-items: center; margin-bottom: 0.5rem; margin-top:0.25rem">
            <div style="margin-right: 0rem; margin-left:0rem; margin-bottom: -20px;">
        ...
        if show_landing_message and featured_image_base64:
            ...
            header_content += f"""
            <div style="flex: 1; min-width: 200px; margin-left: 0.5rem; margin-bottom: -3rem; align-self: flex-end; padding-bottom: 20px;">"""
        else:
            # When no featured image, keep normal centering
            header_content += f"""
            <div style="flex: 1; min-width: 200px; margin-left: 0.5rem; margin-bottom: -0.75rem; margin-top:0.5rem;">"""
        
        # Add featured image if this is the first time and image exists
        if show_landing_message and featured_image_base64:
            header_content += f"""
                <div style="margin-bottom: -1rem; margin-top: -1.2rem;">
                    <img src="data:image/png;base64,{featured_image_base64}" style="max-width: 100%; width: auto; max-height: 70px;">
                </div>"""
