    ...
    if os.path.exists(config['banner_path']):
        ...
        if banner_base64:
            st.markdown(f"""<div style="width:100%; text-align:left; ">
                <img src="data:image/png;base64,{banner_base64}" style="width:100%; max-width:210px; margin-top: -20px;">
            </div>
            """, unsafe_allow_html=True)
    else:
        st.markdown(f"### {config['fallback_title']}")
...
def render_sidebar_from_cache():
    ...
    st.markdown(f"""
    <div style="font-size: 0.85rem; color: rgb(163, 168, 184);  margin-top: -100px; margin-bottom: 10px; text-align: left;">
        {update_text}
    </div>
    """, unsafe_allow_html=True)
