...
def display_banner(img_path, max_width=600):
    ...
    if img_base64:
        st.markdown(f"""<div style="display: flex; justify-content: center; width: 100%;">
            <img src="data:image/png;base64,{img_base64}" style="width: 100%; max-width: {max_width}px;  height: auto; margin-top:-70px; margin-bottom:-5px; ">
        </div>
...
def create_deck_section(section_type):
    ...
    if os.path.exists(config['banner_path']):
        ...
        if banner_base64:
            st.markdown(f"""<div style="width:100%; text-align:left; margin-bottom: 0px;">
                <img src="data:image/png;base64,{banner_base64}" style="width:100%; max-width:210px; margin-top: -40px; margin-bottom:-20px;">
            </div>
            """, unsafe_allow_html=True)
    else:
        st.markdown(f"### {config['fallback_title']}")
