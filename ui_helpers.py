    ...
    try:
        ...
        if header_image:
            ...
            <div style="width: 100%; margin-top: -18px; margin-bottom: 7px; position: relative;">
                ...
                <div style="position: absolute; bottom: 0px; right: 0px; background-color: rgba(0, 0, 0, 0.8); color: white; padding: 2px 4px; border-radius: 4px 0px 4px 0px; font-size: 0.65rem; font-weight: 500;">
                    {stats_text}
                </div>
            </div>
            """, unsafe_allow_html=True)       
...
def create_deck_section(section_type):
    ...
    if header_image:
        ...
        <div style="width: 100%; margin-top: -18px; position: relative;">
            ...
            <div style="position: absolute; bottom: 0px; right: 0px; background-color: rgba(0, 0, 0, 0.8); color: white; padding: 2px 4px; margin-bottom: -7px; border-radius: 4px 0px 0px 0px; font-size: 0.65rem; font-weight: 500;">
                {stats_text}
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def display_counter_picker_sidebar():
    ...
    if os.path.exists(config['banner_path']):
        ...
        if banner_base64:
            st.markdown(f"""<div style="width:100%; text-align:center;">               
                <img src="data:image/png;base64,{banner_base64}" style="width:100%; max-width:350px; margin-top: -30px; margin-bottom:10px;">
            </div>
            """, unsafe_allow_html=True)
    else:
...
def render_sidebar_from_cache():
    ...
    st.markdown(f"""
    <div style="font-size: 0.85rem; color: rgb(163, 168, 184); margin-top: -50px; margin-bottom: -20px; text-align: left;">
        {update_text}
    </div>
    """, unsafe_allow_html=True)
